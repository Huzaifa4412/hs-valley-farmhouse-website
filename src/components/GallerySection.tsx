...
export function GallerySection({ onOpenLightbox }: GallerySectionProps) {
  ...
  return (
    <section id="gallery" className="section-space bg-[#0B0F0D]">
      <div className="page-container">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 mb-7">
          <div><p className="eyebrow mb-3">See it for yourself</p><h2 className="section-title">The property <em className="text-[#B99A5B]">in pictures.</em></h2></div>
          <p className="text-sm text-[#FAF9F5]/65 max-w-xs">Real photographs from around HS Valley. Select a photo for a closer look.</p>
        </div>
        <div className="flex flex-wrap gap-2 mb-6" role="group" aria-label="Filter property photos">
        ...
        <div className="grid grid-cols-1 min-[400px]:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {visibleItems.map(item => <button key={item.id} type="button" aria-label={`View ${item.title}`}
            onClick={() => onOpenLightbox(galleryItemsData.findIndex(g => g.id === item.id))}
            className="gallery-card group text-left rounded-2xl overflow-hidden bg-[#14251D]/50 border border-[#FAF9F5]/10 hover:border-[#B99A5B]/60 transition-colors">
            <div className="aspect-[4/3] overflow-hidden relative">
              ...
              <span className="absolute right-3 top-3 rounded-full bg-[#0B0F0D]/70 p-2 text-[#FAF9F5]" aria-hidden="true"><ArrowUpRight size={18} /></span>
            </div>
            <div className="px-4 py-4 sm:px-5"><p className="text-[10px] uppercase tracking-[0.15em] text-[#B99A5B] mb-1">{categories.find(c => c.id === item.category)?.label}</p><h3 className="text-lg sm:text-xl font-serif-editorial leading-tight">{item.title}</h3></div>
          </button>)}
        </div>
        {visibleItems.length < filteredItems.length && <div className="text-center mt-8"><button type="button" onClick={() => setShowAll(true)} className="inline-flex items-center gap-3 min-h-12 px-6 py-3 border border-[#B99A5B]/50 rounded-full text-sm text-[#B99A5B] hover:bg-[#B99A5B] hover:text-[#0B0F0D] transition-colors"><Plus size={17} /> Show all {filteredItems.length} photographs</button></div>}
      </div>
    </section>
  );
}
